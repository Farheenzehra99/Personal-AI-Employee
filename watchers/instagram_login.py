#!/usr/bin/env python3
"""
Instagram Login Helper - Opens Instagram for manual login
Usage: python watchers/instagram_login.py --session-path .instagram_session
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Instagram Login Helper')
    parser.add_argument('--session-path', default='.instagram_session',
    ...
    print("2. Login to Instagram with your account")
    ...
    with sync_playwright() as p:
        ...
        for i in range(args.timeout):
            ...
            # Check if logged in
            if 'instagram.com' in current_url and 'login' not in current_url and 'checkpoint' not in current_url:
                # Wait a bit more to confirm
                time.sleep(3)
                if 'feed' in page.url or 'explore' in page.url or 'reels' in page.url:
                    print("")
